...
async def list_incidents():
    try:
        response = await stub.ListIncidents(incident_pb2.Empty())
...
async def create_incident(title: str, description: str, severity: str, ci_id: int):
    try:
        response = await stub.CreateIncident(incident_pb2.CreateIncidentRequest(
...
async def add_incident_update(incident_id: int, text: str):
    try:
        response = await stub.AddIncidentUpdate(incident_pb2.AddIncidentUpdateRequest(
