...
def run():
    ...
    with grpc.insecure_channel("localhost:50052") as cmdb_channel:
        ...
        with grpc.insecure_channel("localhost:50053") as incident_channel:
            ...
            incident_stub.AddIncidentUpdate(incident_pb2.AddIncidentUpdateRequest(
                incident_id=incident.id, text="The issue has been fixed, checkout is working again"
            ))

            elapsed = 0
            fetched = incident_stub.GetIncident(incident_pb2.IncidentIdRequest(id=incident.id))
            while not fetched.ai_suggested_status and elapsed < timeout:
                time.sleep(poll_interval)
                elapsed += poll_interval
                fetched = incident_stub.GetIncident(incident_pb2.IncidentIdRequest(id=incident.id))

            print(f"ai_suggested_status: {fetched.ai_suggested_status}")
